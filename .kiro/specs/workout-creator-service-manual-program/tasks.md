...
- [ ] 1. Create domain models for manual program creation
  - [ ] 1.1 Create `DayAssignmentType` enum and `DayAssignment` record in `vault/domain/`
  ...
  - [ ] 1.2 Create `ManualProgram` record in `vault/domain/`
...
- [ ] 2. Create inbound port and command
  - [ ] 2.1 Create `CreateManualProgramCommand` record in `vault/ports/inbound/`
    - Fields: `programName` (String), `ownerUserId` (String), `dayAssignments` (List<DayAssignment>)
    - _Requirements: 1.1_

  - [ ] 2.2 Create `CreateManualProgramUseCase` interface in `vault/ports/inbound/`
    - Single method: `UUID createManualProgram(CreateManualProgramCommand command)`
    - _Requirements: 1.1, 1.2_

- [ ] 3. Extend outbound port and create exception
  - [ ] 3.1 Add `saveManualProgram(ManualProgram)` method to existing `VaultProgramRepository` interface
    - Returns `void` — the caller already has the generated UUID
    - _Requirements: 5.1, 5.2_

  - [ ] 3.2 Create `WorkoutNotFoundException` in `common/exception/`
...
- [ ] 4. Implement application service logic
  - [ ] 4.1 Add `CreateManualProgramUseCase` implementation to existing `VaultService`
  ...
  - [ ]* 4.2 Write unit tests for `VaultService.createManualProgram`
...
- [ ] 5. Checkpoint - Ensure domain, ports, and service compile
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 6. Create Flyway migration and JPA adapter
  - [ ] 6.1 Create Flyway migration `V103__create_day_assignments.sql`
  ...
  - [ ] 6.2 Create `DayAssignmentJpaEntity` in `vault/adapters/outbound/`
  ...
  - [ ] 6.3 Implement `saveManualProgram` in `JpaVaultProgramRepository`
...
- [ ] 7. Create request/response DTOs and controller endpoint
  - [ ] 7.1 Create `CreateManualProgramRequest` and `DayAssignmentRequest` DTOs in `vault/adapters/inbound/dto/`
  ...
  - [ ] 7.2 Create `CreateProgramResponse` record in `vault/adapters/inbound/dto/`
    - Single field: `String id`
    - _Requirements: 1.2_

  - [ ] 7.3 Add `createManualProgram` POST endpoint to `VaultController`
  ...
  - [ ] 7.4 Add `WorkoutNotFoundException` handler to `GlobalExceptionHandler`
...
- [ ] 8. Checkpoint - Ensure build compiles and existing tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 9. Write property-based tests
  - [ ]* 9.1 Write property test: whitespace-only program names are rejected
  ...
  - [ ]* 9.2 Write property test: conditional required field validation
  ...
  - [ ]* 9.3 Write property test: non-positive day numbers are rejected
  ...
  - [ ]* 9.4 Write property test: duplicate day numbers are rejected
  ...
  - [ ]* 9.5 Write property test: non-existent workout references are rejected
  ...
  - [ ]* 9.6 Write property test: manual program persistence round-trip
...
- [ ] 10. Write integration test for the full endpoint
  - [ ]* 10.1 Write integration test for `POST /api/v1/vault/programs`
